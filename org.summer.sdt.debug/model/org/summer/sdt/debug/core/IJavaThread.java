...
public interface IJavaThread extends IThread, IFilteredStep {
	...
	/**
	 * Invokes the given evaluation with the specified progress monitor. This thread fires a resume event when the evaluation begins, and a suspend
	 * event when the evaluation completes or throws an exception. The events are given a detail as specified by <code>evaluationDetail</code> (one of
	 * <code>DebugEvent.EVALUATION</code> or <code>DebugEvent.EVALUATION_IMPLICIT</code>).
	 * <p>
	 * Since 3.5, the <code>org.summer.sdt.debug.breakpointListeners</code> extension point supports evaluation execution during a listener call back.
	 * Suspend and resume events are not fired during listener call backs. Unspecified model specific events are fired.
	 * </p>
	 * 
	 * @param evaluation
	 ...
	 *            one of <code>DebugEvent.EVALUATION</code> or <code>DebugEvent.EVALUATION_IMPLICIT</code>
	 * @param hitBreakpoints
	 *            whether or not breakpoints should be honored in this thread during the evaluation. If <code>false</code>, breakpoints hit in this
	 *            thread during the evaluation will be ignored.
	 * @exception DebugException
